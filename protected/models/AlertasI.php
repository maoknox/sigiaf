...
class Asistencia extends CActiveRecord
{
	...
	public function reporteAsistencia(){
		...
		header("Content-Disposition: filename=repAsist-".$anio."-".$mes.".xls");  
		...
		$sqlConsAreaIns="select * from area_inscripcion order by id_areainscr,area_interes asc";
		...
		$sqlConsAdol="select * from adolescente as a left join numero_carpeta as b on a.num_doc=b.num_doc order by id_numero_carpeta asc";
		...
		$fechaIni = "01/".$mes."/".$anio;
		$fechaFin = $ultimo_dia."/".$mes."/".$anio;
		foreach($repAsistAdol as $pk=>$asistAdol){
			$semana =1;
			$dia = 1;
