...
<!-- Text input-->
    <div class="form-group">
	   	<?php echo $formCreaRol->labelEx($modeloRol,'nombre_rol',array('class'=>'col-md-4 control-label','for'=>'searchinput'));?>
        <div class="col-md-4">
            <?php echo $formCreaRol->textField($modeloRol,'nombre_rol',array('class'=>'form-control input-md'));?>
            <?php echo $formCreaRol->error($modeloRol,'nombre_rol',array('style' => 'color:#F00'));?>     
    	</div>
    </div>
    <div class="form-group">
	   	<?php echo $formCreaRol->labelEx($modeloRol,'descripcion_rol',array('class'=>'col-md-4 control-label','for'=>'searchinput'));?>
        <div class="col-md-4">
            <?php echo $formCreaRol->textArea($modeloRol,'descripcion_rol',array('class'=>'form-control input-md'));?>
            <?php echo $formCreaRol->error($modeloRol,'descripcion_rol',array('style' => 'color:#F00'));?>  
            <?php $modeloRol->id_rol=0; echo $formCreaRol->hiddenField($modeloRol,'id_rol',array('class'=>'form-control input-md'));?>
    	</div> 
    </div>
    <div class="form-group">
					...
					foreach($menuHijo as $menuHijoAux){
						...
						if(!empty($menuHijoHijo)){
							...
							foreach($menuHijoHijo as $menuHijoHijoAux){
								...
								?>--<?php echo $menuHijoHijoAux['titulo_menu'];?><input type="checkbox" id="<?php echo $menAuxAux['id_menu'].$menuHijoAux['id_menu'].$menuHijoHijoAux['id_menu']?>" name="Menu[<?php echo $menAuxAux['id_menu'];?>][<?php echo $menuHijoAux['id_menu']?>][<?php echo $menuHijoHijoAux['id_menu']?>]" /><br /><?php
							}
						}
						else{
							?>-<?php echo $menuHijoAux['titulo_menu'];?><input type="checkbox" id="<?php echo $menAuxAux['id_menu'].$menuHijoAux['id_menu']?>" name="Menu[<?php echo $menAuxAux['id_menu'];?>][<?php echo $menuHijoAux['id_menu']?>]" /><br /><?php
						}
					}
				}
			//echo "<hr/>";
						...
						'success' => 'function(datosRef) {	
							...
							if(datosRef.estadoComu=="exito"){
								if(datosRef.resultado=="exito"){
									...
									jAlert("Rol creado satisfactoriamente", "Mensaje");
